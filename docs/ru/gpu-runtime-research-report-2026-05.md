# Исследовательский отчёт по среде выполнения GPU

**Тема:** RTX 6000 Ada vs RTX 5090 для long-context LLM inference
**Период исследования:** май 2026
**Автор:** Михаил Степанов
**Статус:** технический исследовательский отчёт
...
Это полный технический исследовательский отчёт. Отдельное краткое резюме для руководства может быть составлено на его основе.
...
Метрики Node Exporter (`node_cpu_seconds_total`, `node_load1`, `node_load5`, `node_load15`, `node_memory_*`, `node_disk_*`, `node_network_*`) также были доступны для валидации на уровне хоста. Они использовались для проверки того, не искажаются ли результаты инференса ограничениями CPU, RAM, диска или сети на стороне хоста; полные выводы — в Разделе 13.7.
...
**Рисунок 10.1: Dual RTX 5090 topology probe, теплое p50 vs хвосты холодного бэкенда.** Источник: `normalized-results.md` Таблица 1 и private dual-run artifacts. Подтверждает: топология dual 5090 дала client-side timing data с резким расхождением между warm-cache p50 и cold-backend tail latency. Оговорка: для этого прогона отсутствуют DCGM, TTFT, per-request route attribution, а метрики llama.cpp недействительны; используйте только как exploratory topology evidence.
...
- **Доказательства завершения запросов**: приватные curl metric artifacts для сохранённых c4/c6 dual-5090 benchmark requests показывают HTTP 200 и пустые curl error files. Это подтверждает завершение запросов для этих сохранённых строк, но не даёт per-request backend attribution.

### 10.8 Что демонстрирует этот прогон

- Топология обслуживала client benchmark requests через единый OpenAI-совместимый эндпоинт, опирающийся на два независимых бэкенда `llama.cpp` на одном инстансе с использованием стратегии `least-busy`.
- Приватные run artifacts показывают, что backend-и были сконфигурированы раздельно (`CUDA_VISIBLE_DEVICES=0` на порту 18001 и `CUDA_VISIBLE_DEVICES=1` на порту 18002), а inline `nvidia-smi` samples показывают высокую утилизацию обеих GPU во время конкурентных фаз. Это поддерживает функциональное использование обоих backend на уровне batch.
...
### 13.7 Host-level метрики платформы

Помимо GPU/DCGM-телеметрии, на benchmark-хостах были доступны стандартные метрики Node Exporter. Они использовались для проверки того, не искажаются ли результаты инференса ограничениями со стороны CPU, RAM, диска или сети.

В активные окна инференса RTX 5090 метрика `node_load1` находилась примерно в диапазоне `0.5–1.0` на хосте с 72 логическими CPU, что указывает на пренебрежимо малую CPU-нагрузку для измеренного пути llama.cpp + LiteLLM. В окне Ada 6000 также не наблюдалось устойчивого CPU pressure.

Оперативная память оставалась в безопасной зоне. На хосте RTX 5090 было около 64 GiB RAM, оценочно занятая память держалась примерно в диапазоне `10–18 GiB` во время активных benchmark-окон. Хост Ada 6000 показал более высокое абсолютное потребление RAM — примерно `20–31 GiB` — на хосте с большим объёмом системной памяти. Так как веса модели и KV-cache находились в VRAM, системная RAM использовалась в основном ОС, LiteLLM, экспортерами, логами и небольшими буферами — в обоих случаях далеко от потолка.

Disk I/O во время steady-state генерации токенов был близок к нулю — только короткие всплески записи/чтения, связанные с логами, метриками и стартом. Признаков устойчивого давления на диск во время inference-окон не наблюдалось.

Сетевые метрики были доступны, но нагрузочный клиент, LiteLLM и llama.cpp работали на одном хосте. Внешняя сетевая пропускная способность, latency удалённых клиентов, TLS/API gateway overhead и production ingress behavior этим исследованием существенно не проверялись.

cAdvisor и полные `container_*` метрики не собирались, поэтому исследование не даёт per-container атрибуции CPU, RAM, filesystem и network usage.

![Рисунок 13.1: Grafana snapshots хоста и GPU](../../charts/ru/grafana-host-gpu-telemetry-snapshots.png)

**Рисунок 13.1: Grafana snapshots для репрезентативных benchmark-окон Ada 6000 и RTX 5090.** GPU utilization, VRAM, power и temperature показывают активную inference-нагрузку. Панели Node Exporter показывают, что CPU, RAM, диск и сеть хоста не были устойчивыми bottleneck. Скриншоты носят иллюстративный характер; числовые выводы основаны на экспортированных метриках и нормализованных summary.

В целом host-level телеметрия не показала признаков того, что CPU, RAM, диск или сеть были bottleneck в измеренных benchmark-нагрузках. Основные ограничения оставались на стороне GPU и runtime: VRAM, размер контекста, загрузка GPU, latency, routing behavior и выбор model/profile.
...
| исследовательский зонд топологии dual-backend | 2x RTX 5090 | Два бэкенда Q4_K_XL `200k / 2` за маршрутизацией LiteLLM least-busy | Раздел 10.8 сообщает о таймингах на стороне клиента для трафика, отправленного через два независимых бэкенда; Раздел 10.4 показывает быстрые кейсы p50 с теплым кэшем. | Нет DCGM, нет TTFT, нет per-request route attribution, недействительные метрики llama.cpp; Разделы 10.5-10.7 и 13.2 показывают асимметрию прогрева, хвосты в 70-134 с для холодных бэкендов, отсутствие телеметрии и слепую зону метрики deferred. |
...
Метрики Node Exporter на уровне хоста были доступны и не показали CPU/RAM/disk bottleneck в измеренных inference-окнах (Раздел 13.7). Однако это не был полный production platform load test: нет cAdvisor или per-container атрибуции, нет внешнего многопользовательского трафика, нет RAG/vector DB нагрузки, нет API gateway или auth layer overhead, нет длительного soak test.
...
Прогон с двойной RTX 5090 является исследовательским. Он предоставляет наблюдения за таймингами на стороне клиента для маршрутизируемой топологии, показывает поведение теплого кэша, а приватные артефакты подтверждают HTTP 200 completion для сохранённых c4/c6 request rows. Прогон всё ещё чувствителен к асимметрии прогрева и размещению кэша, и ему не хватает полных метрик DCGM, LiteLLM TTFT, per-request route attribution и валидных метрик llama.cpp. К нему следует относиться как к исследовательскому зонду топологии и диагностической цели, а не как к доказательству баланса маршрутизации или финальной базовой линии пропускной способности (capacity baseline).
...
*Нет DCGM, нет TTFT, нет per-request route attribution. Характеристика основана на client-side timings, HTTP status files и грубых GPU samples.*
